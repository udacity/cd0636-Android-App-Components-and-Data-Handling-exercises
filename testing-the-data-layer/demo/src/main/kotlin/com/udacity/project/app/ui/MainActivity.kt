...
class MainActivity : AppCompatActivity() {
    private val viewModel: CountryViewModel by viewModels {
        ...
        val repository = DefaultCountryRepository(database.countryDao(), CountriesService.api)
    ...
    private fun setupClickListeners() {
        syncButton.setOnClickListener { viewModel.syncCountries() }
    }

    private fun collectFlows() {
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                launch { viewModel.countries.collect { countryAdapter.updateCountries(it) } }
                launch { viewModel.totalCount.collect { totalCountriesTextView.text = "Total: $it countries" } }
                launch { viewModel.syncState.collect { syncStatusTextView.text = it } }
            }
        }
    }
}
